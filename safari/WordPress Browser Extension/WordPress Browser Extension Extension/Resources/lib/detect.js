...
(function () {
  ...
  function detectWordPress(doc, options) {
    ...
    const result = {
      ...
      context: {
        ...
        // (issue #33). Derived from the REST API root below; null until then.
        baseUrl: null,
    ...
    if (docOrigin && docOrigin !== 'null') {
      result.context.baseUrl = deriveBaseUrl(
  ...
  function deriveBaseUrl(origin, restApiRoot, adminPathname) {
    if (!restApiRoot) {
      ...
      if (prefix !== null) {
        ...
        try {
          ...
          if (safeProtocol && candidate.origin === originUrl.origin) {
            ...
            return path ? `${candidate.origin}${path}` : candidate.origin;
          }
    ...
    try {
      ...
      if (!safeProtocol || rootUrl.origin !== originUrl.origin) return origin;
      ...
      return path ? `${rootUrl.origin}${path}` : rootUrl.origin;
    } catch (_) {
      return origin;
    }
  }
